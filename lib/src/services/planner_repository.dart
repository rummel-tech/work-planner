...
class PlannerRepository {
  ...
  Future<DayPlanner?> getDayPlannerByDate(DateTime date) async {
    ...
    final records = await _dayStore.find(_db,
        finder: Finder(filter: Filter.equals('date', normalized.toIso8601String())));
  ...
  Future<WeekPlanner?> getWeekPlannerByStartDate(DateTime startDate) async {
    ...
    final records = await _weekStore.find(_db,
        finder: Finder(filter: Filter.equals('weekStartDate', normalized.toIso8601String())));
    if (records.isEmpty) return null;
  ...
  Future<WeekPlanner> updateWeekPlannerGoals(DateTime weekStart, List<String> goals) async {
  ...
  Future<WeekPlanner> updateWeekPlannerNotes(DateTime weekStart, String notes) async {
    final normalized = _weekStart(weekStart);
    final dateStr = _dateStr(normalized);
  ...
  WeekPlanner _weekPlannerFromApiJson(Map<String, dynamic> json, DateTime weekStart) {
    return WeekPlanner.create(
      id: json['id'] as String,
