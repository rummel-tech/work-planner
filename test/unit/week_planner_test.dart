  ...
  group('WeekPlanner', () {
    group('WeekPlanner.create', () {
      ...
      test('defaults dailyPlannerEntries to empty', () {
        expect(WeekPlanner.create(weekStartDate: monday).dailyPlannerEntries,
            isEmpty);
      });
    });
    ...
    group('addWeeklyGoal / removeWeeklyGoal', () {
      test('addWeeklyGoal appends goal', () {
        final wp = WeekPlanner.create(weekStartDate: monday);
        expect(wp.addWeeklyGoal('Ship feature').weeklyGoals,
            equals(['Ship feature']));
      });
    ...
    group('addDailyPlannerEntry / removeDailyPlannerEntry / getDayPlannerId', () {
      ...
      test('addDailyPlannerEntry replaces existing entry for same day', () {
        final wp = WeekPlanner.create(weekStartDate: monday)
            .addDailyPlannerEntry(0, 'old')
            .addDailyPlannerEntry(0, 'new');
    ...
    group('copyWith', () {
      ...
      test('updates notes', () {
        expect(
          WeekPlanner.create(weekStartDate: monday)
              .copyWith(notes: 'note')
              .notes,
          equals('note'));
      });
    ...
    group('equality', () {
      test('same id means equal', () {
        final a = WeekPlanner.create(
            id: 'same', weekStartDate: DateTime(2025, 1, 6));
        final b = WeekPlanner.create(
            id: 'same', weekStartDate: DateTime(2025, 2, 3));
        expect(a, equals(b));
      });
    });
  });
